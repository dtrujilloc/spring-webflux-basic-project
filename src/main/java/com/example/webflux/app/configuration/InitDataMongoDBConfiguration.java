...
public class InitDataMongoDBConfiguration implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<Product> productInitList = Arrays.asList(
                ...
                Product.builder().name("TV LG").price(200.10).quantity(10).category(electronicCategory).build(),
                Product.builder().name("Apple 15").price(200.10).quantity(4).category(electronicCategory).build(),
                Product.builder().name("Xiaomi pro 8").price(200.10).category(electronicCategory).quantity(7).build(),
                Product.builder().name("Basketball net").price(10.10).category(sportCategory).quantity(20).build(),
                Product.builder().name("Table").price(80.10).quantity(2).category(homeCategory).build()
        );
